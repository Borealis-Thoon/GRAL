...
internal static class Harness
{
    ...
    static int[] ManyGroups() => Enumerable.Range(1, 297).Concat(new[] { 8760, 40000, int.MaxValue }).ToArray();
    static void Sources(string kind)
    {
        ...
        File.WriteAllLines(file, Enumerable.Repeat("header", headerCount).Concat(groups.Select(row)).Append(row(50000)));
        StaticReader(cls);
    ...
    static float[][][][] Float4(int nx, int ny, int nz, int sg) => Enumerable.Range(0, nx).Select(_ => Enumerable.Range(0, ny).Select(_ => Enumerable.Range(0, nz).Select(_ => new float[sg]).ToArray()).ToArray()).ToArray();
    static double[][][] Double3(int nx, int ny, int sg) => Enumerable.Range(0, nx).Select(_ => Enumerable.Range(0, ny).Select(_ => new double[sg]).ToArray()).ToArray();
    ...
    {
        var groups = new[] { 1, 99, 100, 101, 102, 255, 256, 744, 8760, 40000, int.MaxValue }; OutputSetup(groups);
        ...
        if (zipped)
        {
            ...
            {
                for (int s = 1; s <= 9; s++) { var entry = archive.GetEntry($"{weather:00000}-{s}{groups[g]:00}.con"); Check(entry != null, "zip con filename"); using var ms = new MemoryStream(); using (var es = entry!.Open()) es.CopyTo(ms); ms.Position = 0; VerifyGrid(ms, g + 1); }
                var dep = archive.GetEntry($"{weather:00000}-{groups[g]:00}.dep"); Check(dep != null, "zip dep filename"); using var dm = new MemoryStream(); using (var ds = dep!.Open()) ds.CopyTo(dm); dm.Position = 0; VerifyGrid(dm, g + 2);
            }
        }
        else
        {
            for (int g = 0; g < groups.Length; g++)
            {
                for (int s = 1; s <= 9; s++) { string file = $"{weather:00000}-{s}{groups[g]:00}.con"; Check(File.Exists(file), "con filename " + file); VerifyGrid(File.OpenRead(file), g + 1); }
                string dep = $"{weather:00000}-{groups[g]:00}.dep"; Check(File.Exists(dep), "dep filename"); VerifyGrid(File.OpenRead(dep), g + 2);
            }
        }
    }
    static void OdourNames()
    {
        ...
        int[] groups = { 99, 100, 256, 40000, int.MaxValue }; OutputSetup(groups, 0); GP.Odour = true;
        new ProgramWriters().Write2DConcentrations(4, "unused.grz");
        foreach (int g in groups) for (int s = 1; s <= 9; s++) { string file = $"00004-{s}{g:00}.odr"; Check(File.Exists(file), "odour filename " + file); Check(new FileInfo(file).Length == 4, "odour empty fixture header"); }
        GP.Odour = false;
    }
    static int Main(string[] args)
    {
        ...
        Test("configuration_sparse_int32", () => { Config(new[] { 40000, 1, int.MaxValue, 256 }); Check(GP.SourceGroups.SequenceEqual(new[] { 40000, 1, int.MaxValue, 256 }), "ordering"); Check(GP.DecayRate.Length == 4, "sparse compact decay"); Check(GP.Get_Internal_SG_Number(int.MaxValue) == 2, "int32 mapping"); Check(GP.Get_Internal_SG_Number(2) == -1, "unknown mapping"); });
        ...
        Test("configuration_invalid_atomic", () => { Config(new[] { 40000, 1 }); foreach (string value in new[] { "1,1", "0,1", "-1,1", "2147483648", "abc", "", "! empty" }) { Reject(() => GP.ConfigureSourceGroups(value), value); Check(GP.SourceGroups.SequenceEqual(new[] { 40000, 1 }), "invalid mutated list"); Check(GP.Get_Internal_SG_Number(40000) == 0, "invalid mutated map"); } });
        Test("configuration_comments", () => { GP.ConfigureSourceGroups("40000; 1;2147483647 ! 2,2"); Check(GP.SourceGroups.Count == 3, "comments"); });
        foreach (string kind in new[] { "point", "line", "area", "portal" }) Test(kind + "_300_sources", () => Sources(kind));
        Test("decay_sparse_headers", () => { Config(new[] { 40000, 1, int.MaxValue, 256 }); File.WriteAllText("Pollutant.txt", "NOx\n0\n0\n0.05\n40000:0.2\t2147483647:0.3\t123456:9\n"); new ProgramReaders().ReadPollutantTXT(); Check(GP.DecayRate.SequenceEqual(new[] { .2, .05, .3, .05 }), "decay mapping/defaults"); });
        Test("timeseries_sparse_reverse_unknown", () => { Config(new[] { 40000, 1, int.MaxValue }); GP.ISTATIONAER = 0; File.WriteAllText("emissions_timeseries.txt", "Day,Hour,99999,2147483647,1,40000\n1,0,7,0.25,0.5,0.75\n1,1,8,1,0,2\n"); new ProgramReaders().ReadEmissionTimeseries(); Check(GP.EmFacTimeSeries[0, 0] == .75f, "sg40000"); Check(GP.EmFacTimeSeries[0, 1] == .5f, "sg1"); Check(GP.EmFacTimeSeries[0, 2] == .25f, "sgmax"); Check(GP.EmFacTimeSeries[1, 0] == 2, "hour2"); });
